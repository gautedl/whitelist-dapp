...
export default function Home() {
  // walletConnected keep track of whether the users's wallet is connected or not
  ...
  const getProviderOrSigner = async (needSigner = false) => {
    ...
    // If user is not connected to the Goerli network, we need to access the "current" value to get access to the underlying object
  ...
  const addAddressToWhitelist = async () => {
    try {
      ...
      setLoading(true);
      // wait for the transaction to get mines
      await tx.wait();
      setLoading(false);
  ...
  /**
   * getNumberOfWhitelisted: gets the number og whitelisted addresses
   */
  ...
  const checkIfAddressInWhitelist = async () => {
    try {
      ...
      // Get the address associated to the signer which is connectet to metamask
